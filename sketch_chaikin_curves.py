...
class ChaikinCurvesSketch(vsketch.SketchClass):
    ...
    def generate_points(self, vsk: vsketch.Vsketch):
        ...
        if self.closed:
            ...
            polar_points = [(vsk.random(
                0,
                min(abs(self.width / (2 * math.cos(theta))),
                    abs(self.height / (2 * math.sin(theta))))), theta)
                            for theta in thetas]
            points = [
                Point(r * math.cos(theta) + self.origin.x,
                      r * math.sin(theta) + self.origin.y)
            ...
            xs = [
                round(vsk.random(self.width), self.precision)
                for _ in range(self.num_points)
            ]
            xs.sort()
            points = [Point(x, vsk.random(self.height)) for x in xs]
        return [
            Point(round(point.x, self.precision),
                  round(point.y, self.precision)) for point in points
        ]
